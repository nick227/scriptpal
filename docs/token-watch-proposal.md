...
- Update `public/js/init-clean.js` to instantiate the token widget right after `renderSharedTopBar()` and wire it similarly to the auth widget (pass shared elements, `stateManager`, `eventManager`, and `ScriptPalAPI`).
