...
3. **Script services** (`server/controllers/scripts/AppendPageService.js`, `FullScriptService.js`, `ScriptVersionService.js`) still call `AppendPageChain` and `FullScriptChain` directly to support append/full-script generation workflows triggered from `chatController.startChat`.
