...
2. **Routing** – The resolved intent drives `router.route`, which now only resolves `AppendScriptChain` (for `SCRIPT_CONVERSATION`) or `DefaultChain` (for `GENERAL_CONVERSATION`). Only the script-conversation path includes the script content before the AI call; the other path keeps the exchange lightweight and history-free.
