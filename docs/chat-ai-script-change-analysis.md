...
- `Chat.processMessage` loads the script via `ScriptManager.getScript`, decides whether to treat the request as `EDIT_SCRIPT` or general conversation, prepares the enriched context plus chat history, routes to the appropriate chain, and saves the assistant reply (`server/controllers/chat/Chat.js:14`, `server/controllers/scripts/ScriptManager.js:4`, `server/controllers/chat/ChatHistoryManager.js:9`).
